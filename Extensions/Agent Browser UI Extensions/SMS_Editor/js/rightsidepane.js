var iconFont = "fas fa-envelope";
var iconColor = "#259fed";
var createIcon = "font awesome";
var rightSidePaneURL = "../view/pane.html";
var rightSidePaneLabel = "SMS Editor";
var appName = "SMS_Editor";
var appVersion = "1.0";
...
var constants, rightPane, _extensionProvider, gContext, sessionToken, restUrl, incID, incType = "", catID;
...
function viewChangeHandler(parameter) {
    ...
    if (rightPane) {
        rightPane.isExpanded().then(function (status) {
            console.log(status);
            paneFrameId = null;
            if (!paneFrameId && localStorage.getItem("SMS_Editor_Pane_FrameId")) {
                paneFrameId = localStorage.getItem("SMS_Editor_Pane_FrameId");
                localStorage.removeItem("SMS_Editor_Pane_FrameId");
            }
            if (paneFrameId) {
                if (ieFlag) {
                ...
                localStorage.setItem("SMS_Editor_Incident_" + incID, JSON.stringify(info));
            }
            rightPane.dispose();
            rightPane = null;
...
function registerUIExtension(parameter) {
    _extensionProvider.registerUserInterfaceExtension(function (IUserInterfaceContext) {
        IUserInterfaceContext.getLeftSidePaneContext().then(function (ISidePaneContext) {
            ISidePaneContext.getSidePane('smsEditorPane').then(function (rightPanelMenu) {
                console.log(rightPanelMenu);
                localStorage.setItem("SMS_Editor_FrameId", this.window.frameElement.id);
...
function handleWSClose(param) {
    return new ExtensionPromise(function (resolve, reject) {
        ...
        if (param.workspaceRecord.getWorkspaceRecordType() == "Incident") {
            if (localStorage.getItem('SMS_Editor_Incident_' + param.workspaceRecord.getWorkspaceRecordId())) {
                localStorage.removeItem('SMS_Editor_Incident_' + param.workspaceRecord.getWorkspaceRecordId())
            }
        }
